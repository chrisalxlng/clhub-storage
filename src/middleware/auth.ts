...
const keycloakIssuerRealmUrl = `${process.env.CLHUB_STORAGE_KEYCLOAK_ISSUER_URL}/realms/${process.env.KEYCLOAK_APPS_REALM}`;
if (!keycloakIssuerRealmUrl)
	throw new Error(
		"CLHUB_STORAGE_KEYCLOAK_ISSUER_URL and/or KEYCLOAK_APPS_REALM is not set",
	);

const keycloakJwksRealmUrl = `${process.env.CLHUB_STORAGE_KEYCLOAK_JWKS_URL}/realms/${process.env.KEYCLOAK_APPS_REALM}`;
if (!keycloakJwksRealmUrl)
	throw new Error(
		"CLHUB_STORAGE_KEYCLOAK_JWKS_URL and/or KEYCLOAK_APPS_REALM is not set",
	);
